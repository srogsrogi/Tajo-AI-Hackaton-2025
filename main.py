            ...
            async def receive_from_twilio():
                ...
                try:
                    async for msg in websocket.iter_text():
                        ...
                        if data['event'] == 'media':
                            # Check if the WebSocket is open and ready to receive data
                            if openai_ws and not openai_ws.closed:
                                ...
                                logger.warning(
                                    f"Received media but OpenAI WebSocket is not open or is closed. WebSocket state: {openai_ws.closed if openai_ws else 'None'}"
                                )
                    ...
                    # Close the OpenAI WebSocket if it's still open
                    if openai_ws and not openai_ws.closed:
